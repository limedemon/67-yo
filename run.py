...
# ── Инициализация БД на случай раннего веб-запроса ───────────────────────────
...
# ── Точка входа ───────────────────────────────────────────────────────────────
# Один процесс держит и бота (polling), и веб-сервер (mini app). Запускается
# через свой Dockerfile (CMD ["python", "run.py"]) — BotHost не добавляет своих
# процессов, поэтому конфликта портов нет.
async def main():
